...
public class Order_29_31 extends BaseTest {
	
	@Test 
	
	public void testOrder_29_31() {
		
		//To Do List
		//1. login as admin
		//2. Go to add order
		//3. create an order: insert XSS in rate value
		...
		cliName.clear();
		cliName.sendKeys("Italy");
		
		WebElement cliContact=driver.findElement(By.id("clientContact"));
		cliContact.clear();
		cliContact.sendKeys("00000");
		
		Select element = new Select(driver.findElement(By.id("productName1")));
	    element.selectByVisibleText("Swiss Watch");
	    
	    WebElement rateBox= driver.findElement(By.id("rateValue1"));
	    String xss= "1\"><button>free</button><input type=\"hidden";
	    
	    JavascriptExecutor jse = (JavascriptExecutor) driver;
	    jse.executeScript("arguments[0].setAttribute('value', arguments[1])", rateBox, xss);
		...
		WebElement ratevalue1= driver.findElement(By.xpath("/html[1]/body[1]/div[1]/div[2]/div[2]/form[1]/table[1]/tbody[1]/tr[1]/td[2]/button[1]"));
		String strng = ratevalue1.getText();
		assertEquals(strng, ratevalue1.getText());	
